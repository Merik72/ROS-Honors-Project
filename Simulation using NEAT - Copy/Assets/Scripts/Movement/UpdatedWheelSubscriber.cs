...
public class UpdatedWheelSubscriber : MonoBehaviour
{
    ...
    private static float ballsScalar = 5f;//0.25f/8f;
    private static float timeScalar = 0.1f; // scaling up numbers from the PPO because they are toooo small
    ...
    public Contact contact = Contact.NoWall;
    private float contactTimeLimit;
    [SerializeField]
    private float timeDecline;
    ...
    private void Start()
    {
        WHEEL_TOPIC = transform.root.name + "/vel_wheels";
    ...
    void publish()
    {
        ...
        if (contact == Contact.NoWall)
        {
            ...
            */
            ...
            float normalizedDistanceReward = Math.Abs(distanceReward)*spinReward;
            float normalizedForwardsBackwards = Math.Abs(forwardsBackwards) *spinReward;
            ...
            rewardMsg.data = ( displacementReward + veloReward + normalizedDistanceReward + spinReward + rewardForBallTouching + normalizedForwardsBackwards + timeDecline);
        ...
        if (rewardMsg.data < LOWER_LIMIT)
        {
            rewardMsg.data = LOWER_LIMIT;
        }
    ...
    public void RESET_ENV()
    {
        ...
        resetMyself(); // find how to make this work
    ...
    private void FixedUpdate()
    {
        ...
        if (rewardMsg.data == LOWER_LIMIT && !received)
        {
            publish();
            RESET_ENV();
        } else
        ...
        if (going)
        {
            timeDecline -= Time.deltaTime * timeScalar;
        }
