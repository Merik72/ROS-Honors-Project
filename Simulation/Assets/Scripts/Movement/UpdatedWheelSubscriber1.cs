...
public class UpdatedWheelSubscriber1: MonoBehaviour
{
...
//  [Header("Static Reward Components")]
    private static float displacementScalar = 8f;//15f;
    private static float veloScalar = 0f;//25f;
    private static float distanceScalar = 0.5f;//1f;
    private static float spinScalar = 0;//0.25f/8f;
    ...
    public const float ENFORCED_LATENCY = 0f;//*.28f;
    ...
    void publish()
    {
        ...
        if (contact == Contact.NoWall)
        {
            ...
            */
            ...
            float headingRelativeToTarget = Vector3.Angle(chassisControl.transform.forward, lineToTarget);
            ...
            float distanceReward = distanceTravelledToTarget * distanceScalar;

            float rewardForBallTouching = (displacementVSheading > 0) ? ballsTouched : 0;
    ...
    public void RESTART_SCENE()
    {
        ...
        leftVelocity = 0;
        rewardMsg.data = 0;
        chassisControl.TeleportRoot((transform.localPosition = Vector3.zero), startDirection);
