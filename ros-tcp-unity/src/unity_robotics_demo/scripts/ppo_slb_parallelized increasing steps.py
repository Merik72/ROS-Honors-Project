...
def training_callback(image, ppo_agent, robot): 
    ...
    try:
        ...
        #print("robotaction: ", robot.action)
        #print("robotaction: ", robot.action[0], robot.action[1])
        #print("robotaction: ", type(robot.action[0]), type(robot.action[1]))
...
def train(robot):
    ...
    for x in range(num_steps+1):
        ...
        # print("env_" + str(robot.id) + CAMERA_TOPIC)
...
def training_loop():
    ...
    while True:
        ...
        # for x in range(BATCH_SIZE):
        #     distances[x] = rospy.wait_for_message("env_" + str(x) + DISTANCE_TOPIC, Float32)
        # print(distances)
