...
FILE_LOCATION = '/home/admin2/ros-tcp-unity/src/unity_robotics_demo/scripts/checkpoint.pt'
# FILE_LOCATION = '/home/admin2/ros-tcp-unity/old.pt'
...
def run():
    ...
    if(LOADING):
        ppo_agent.policy_old.load_state_dict(torch.load(FILE_LOCATION,weights_only=True))
        ppo_agent.policy.load_state_dict(torch.load(FILE_LOCATION,weights_only=True))
    else:
...
def training_loop():
    ...
    while True:
        ...
        if(epoch % REPORTING_RATE == 0):
            ...
            print(trainingTime - startingTime)
            # AVGPath lengths
        ...
        torch.save(ppo_agent.policy_old.state_dict(), FILE_LOCATION)
