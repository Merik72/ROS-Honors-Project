...
STARTUP_TOPIC = 'start'
NUM_STEPS_MAX = 120
EPOCHS_UNTIL_INCREASE = 1
num_steps = 30 # number of steps taken by every batch
...
def training_callback(image, ppo_agent, robot): 
    ...
    try:
        ...
        if(not (robot.steps == 0)): 
            ...
            robot.reward_buf.append(msg.data)
...
def training_loop():
    ...
    while True:
        ...
        for step in range(NUM_STEPS):
            ...
            rewards = []
            for robot in robots:
                states.append(robot.state_buf[step])
                actions.append(robot.action_buf[step])
                logprobs.append(robot.action_logprobs_buf[step])
                statevals.append(robot.state_val_buf[step])
                rewards.append(robot.reward_buf[step])
            bat_states.append(states)
            bat_actions.append(actions)
            bat_logprobs.append(logprobs)
            bat_statevals.append(statevals)
            bat_rewards.append(rewards)
        ppo_agent.push_values(bat_states, bat_actions, bat_logprobs, bat_statevals, bat_rewards)
        '''
        ...
        temp = 0
        for x in ppo_agent.buffer.rewards:
            temp += x
        avg = temp/(num_steps*BATCH_SIZE)
        print(avg, " is avg")
        ...
        if(avg > rewards_Max or rewards_Max == 0):
            rewards_Max = avg        
            print(rewards_Max, "is rewardsmax")    
            torch.save(ppo_agent.policy.actor, 'actor.pt') ##save model to .pt file
            torch.save(ppo_agent.policy.critic, 'critic.pt') ##save model to .pt file
        '''
        X-axis: epoch count
        Y-axis: average reward over 5 epochs
                - Min-max-average distance over 5 epochs
                - Number of harsh turns taken
                    > Immediately go from negative to positive angular/linear control 
                - avg path length
                    > Use pathtracer to find approximate path length
                - avg distance to next waypoint
                    > messaging
                - avg number of waypoints hit
                    - min dist to ball
                - top speed
                - avg wall clock time per step?
        ''' 

        '''
        check if it hit a new record reward
        '''
        '''py
        ### if GPU memory still crashing

        if (Best_Average_Reward < Current_Average_Reward)
        torch.save(ppo_agent.actor, 'actor.pt')
        torch.save(ppo_agent.critic, 'critic.pt')

        del ppo_agent.actor
        del ppo_agent.critic

        gc.collect()
        torch.cuda.empty_cache()

        ppo_agent.actor = torch.load(actor.pt)
        ppo_agent.critic = torch.load(critic.pt)
        '''
